...
"""# DATASET GENERATION (Muinul + Fatin)"""
...
"""# Helper Function for Plotting Feature Importance (Fatin)"""
...
1.   Logistic Regression (Fatin)
2.   Random Forest (Muinul)
3.   XGBoost (Hada)
4.   MLP (Nasrin)
...
for name, model in models.items():
    ...
    print(f"{name:30} | CV F1: {cv_scores.mean():.3f} (+/- {cv_scores.std():.3f})")

"""# LEARNING CURVES (Nasrin)"""
...
"""# INTERPRETATION USING LLMS (Hada)"""
...
evidence = {
    "dataset_size": 1500,
    "churn_distribution": {0: 776, 1: 724},
    "eda_key_points": [
        "Churn is relatively balanced (52% non-churn, 48% churn).",
        "Demographics (age, gender) show similar distributions across churn groups.",
        "Behavioural/engagement/service variables show clearer differences."
    ],
    "model_results": {
        "Decision Tree (Baseline)": {"accuracy": 0.497, "f1": 0.470, "roc_auc": 0.496},
        "Decision Tree (Tuned)": {"accuracy": 0.567, "f1": 0.572, "roc_auc": 0.604},
        "Logistic Regression (Tuned)": {"accuracy": 0.690, "f1": 0.678, "roc_auc": 0.736},
        "Random Forest (Tuned)": {"accuracy": 0.643, "f1": 0.640, "roc_auc": 0.690},
        "XGBoost (Tuned)": {"accuracy": 0.643, "f1": 0.627, "roc_auc": 0.696},
        "Neural Network (MLP - Tuned)": {"accuracy": 0.643, "f1": 0.611, "roc_auc": 0.695},
    },
    "selected_model": "Logistic Regression (Tuned)",
    "selected_model_reason": "highest F1-score and ROC-AUC, interpretable",
    "feature_importance_interpretation": [
        "slm_sentiment (positive): more negative sentiment increases churn risk",
        "feedback_len (positive): longer feedback indicates dissatisfaction and higher churn risk",
        "support_len (positive): prolonged support interactions increase churn risk",
        "has_support_chat (positive): contacting support is associated with churn risk",
        "spend_per_order_approx (negative): higher spending reduces churn risk"
    ],
    "label_definition": "churn=1 means customer churned, churn=0 means retained"
}

"""Summary of Findings"""

prompt_summary = f"""
You are summarising a data mining report in a calm academic tone.

Evidence:
{evidence}

Task:
Write a short summary of the key findings (EDA + modelling results).
- 1 short paragraph for EDA findings
- 1 short paragraph for model comparison and the selected model
Do not exaggerate. Do not invent numbers.
"""

"""Interpretation of Feature Importance"""

prompt_feat = f"""
You are interpreting feature importance for the selected churn model.

Evidence:
{evidence}

Task:
Explain (in 6-10 bullet points) what the top features imply about churn behaviour.
Rules:
- Use the label definition provided
- Explain positive vs negative direction clearly
- Keep it business-friendly (no heavy ML jargon)
"""
...
Task:
Write Section "7.0 Business Insights" with exactly 3 strategies.
Each strategy must include:
- Strategy name (bold)
- What the model/feature importance shows
- Recommended action (clear operational steps)
- Expected impact on churn/retention

Write in the same formal style as academic reports. Avoid hype.
"""

model = genai.GenerativeModel("models/gemini-flash-latest")

summary_out = model.generate_content(prompt_summary).text
feat_out    = model.generate_content(prompt_feat).text
insight_out = model.generate_content(prompt_insights).text

print("=== LLM Summary of Findings ===\n", summary_out)
print("\n=== LLM Interpretation of Feature Importance ===\n", feat_out)
